...
class WorkloadGenerator(object):
    ...
    def candidate_query_generate(self, query, n_candidate):
        ...
        if self.args.use_sample:
            ...
            data = self.data.values
        bitmap = query.generate_bitmap(sample=data, col2idx=self.cols2idx)

        nrows = len(data)

        if len(predicates.keys()) == 1:
            n_extend_dim = np.ones(n_candidate, dtype=int)
        else:
            n_extend_dim = np.random.randint(low=1, high=len(predicates), size=n_candidate)
        ...
        while count < n_candidate:
            ...
            intersection = inds & bitmap
            union = inds | bitmap
            sim = np.sum(intersection) / np.sum(union)

            if sim > self.args.candidate_sim_threshold:
                ...
                query_candidates.append(complete_q)
                count += 1
                ratio = self.args.vary_ratio
                fail_count = 0
                try_count = 0
...
def parse_arg():
    ...
    args.add_argument('--sample_frac', type=float, default=0.10)
    ...
    args.add_argument('--n_candidate', type=int, default=4)
    args.add_argument('--candidate_sim_threshold', type=float, default=0.8)
    args.add_argument('--sel_ub', type=float, default=0.01)
    args.add_argument('--sel_lb', type=float, default=0.002)
...
if __name__ == '__main__':
    ...
    workload_generator = WorkloadGenerator(args=args, dataset=data, table=table)
    ...
    print('Time Consume : {}'.format(time.time() - since))
    ...
    for idx in Containers:
        query = Containers[idx].query
        candidates = workload_generator.candidate_query_generate(query, args.n_candidate)
        pools = [query] + candidates
        alpha = 1.6
        samples = np.random.zipf(alpha, size=100)
        counter = Counter(samples)
        sorted_counter = sorted(counter.items(), key=lambda x: x[1], reverse=True)
        freq_list = [sorted_counter[i][1] for i in range(args.n_candidate)]
        freq_list = [ele / sum(freq_list) for ele in freq_list]
        cdf = []
        base = 0.0
        for ele in freq_list:
            base += ele
            cdf.append(base)
        Candidates[idx] = (pools, cdf)
    ...
    for timestamp in range(args.period):
        ...
        for idx, entity in Containers.items():
            ...
            if freq == 0:
                ...
                batch[idx] = []
                pools, cdf = Candidates[idx][0], Candidates[idx][1]
                for n in range(freq):
                    prob = random.random()
                    for e_id, e in enumerate(cdf):
                        if prob < e:
                            batch[idx].append(pools[e_id])
